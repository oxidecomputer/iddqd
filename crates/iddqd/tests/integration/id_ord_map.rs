...
fn debug_impls() {
    ...
    assert_eq!(
        format!("{map:?}"),
        r#"{SimpleItem { key: 1 }, SimpleItem { key: 10 }, SimpleItem { key: 20 }}"#
    );
    assert_eq!(
...
#[test]
fn and_modify_allows_in_order_key_change() {
    let mut map = IdOrdMap::<TestItem>::make_new();
    ...
    assert!(map.get(&TestKey1::new(&20)).is_none());
    assert_eq!(map.get(&TestKey1::new(&25)).unwrap().value, "bar");
    assert_eq!(
        map.iter().map(|item| item.key1).collect::<Vec<_>>(),
        [10, 25, 30]
    );
}

#[test]
#[should_panic(expected = "key changed during Entry::and_modify")]
...
#[test]
#[should_panic(expected = "key changed during Entry::and_modify")]
fn and_modify_panics_if_first_key_moves_past_successor() {
    let mut map = IdOrdMap::<TestItem>::make_new();
    map.insert_unique(TestItem::new(10, 'a', "x", "foo")).unwrap();
    map.insert_unique(TestItem::new(20, 'b', "y", "bar")).unwrap();

    map.entry(TestKey1::new(&10)).and_modify(|mut item| item.key1 = 21);
}

#[test]
fn retain_allows_in_order_key_change() {
    let mut map = IdOrdMap::<TestItem>::make_new();
    map.insert_unique(TestItem::new(10, 'a', "x", "foo")).unwrap();
    map.insert_unique(TestItem::new(20, 'b', "y", "bar")).unwrap();
    map.insert_unique(TestItem::new(30, 'c', "z", "baz")).unwrap();
...
#[test]
#[should_panic(expected = "key changed during retain")]
fn retain_panics_if_key_changes_out_of_order() {
    let mut map = IdOrdMap::<TestItem>::make_new();
    map.insert_unique(TestItem::new(10, 'a', "x", "foo")).unwrap();
    map.insert_unique(TestItem::new(20, 'b', "y", "bar")).unwrap();
    map.insert_unique(TestItem::new(30, 'c', "z", "baz")).unwrap();
...
#[test]
fn retain_checks_order_against_previous_retained_item() {
    let mut map = IdOrdMap::<TestItem>::make_new();
    map.insert_unique(TestItem::new(10, 'a', "x", "foo")).unwrap();
    map.insert_unique(TestItem::new(20, 'b', "y", "bar")).unwrap();
    map.insert_unique(TestItem::new(30, 'c', "z", "baz")).unwrap();

    // Moving 30 below 20 is fine once 20 has been removed.
    map.retain(|mut item| {
        if item.key1 == 30 {
            item.key1 = 15;
        }
    ...
    map.validate(ValidateCompact::NonCompact, ValidateChaos::No).unwrap();
    assert_eq!(map.iter().map(|item| item.key1).collect::<Vec<_>>(), [10, 15]);
}

#[test]
fn and_modify_and_retain_do_not_hash_keys() {
    #[derive(Debug, PartialEq, Eq, PartialOrd, Ord)]
    struct NoHashKey<'a>(&'a str);

    impl std::hash::Hash for NoHashKey<'_> {
        fn hash<H: std::hash::Hasher>(&self, _state: &mut H) {
            panic!("NoHashKey::hash should not be called");
        }
    }
    ...
    map.insert_unique(Item { id: "a".to_owned(), value: 0 }).unwrap();
    map.insert_unique(Item { id: "b".to_owned(), value: 0 }).unwrap();

    let entry = map.entry(NoHashKey("a")).and_modify(|mut item| item.value = 1);
    let id_ord_map::Entry::Occupied(entry) = entry else {
    ...
    assert_eq!(map.get(&NoHashKey("b")).unwrap().value, 2);

    // get_mut does hash the key.
    let result = catch_panic(|| {
        map.get_mut(&NoHashKey("b")).unwrap().value = 3;
    });
    assert!(result.is_none(), "get_mut should have invoked Hash");
}
